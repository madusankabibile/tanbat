...
class AdBotSeeder extends Seeder
{
    public function run(): void
    {
        $name = 'Daniel Whitmore';

        $bot = User::firstOrNew(['username' => 'daniel_whitmore']);

        $bot->name     = $name;
        $bot->age      = $bot->age      ?? 31;
        $bot->gender   = $bot->gender   ?? 'male';
        $bot->country  = $bot->country  ?? 'GB';
        $bot->email    = $bot->email    ?? 'adbot@nexus.com';
        $bot->password = $bot->password ?? bcrypt(Str::random(40));
        $bot->bio      = 'Surfacing the deals and offers worth a second look.';
        $bot->role     = 'user';
