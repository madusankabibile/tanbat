const _=window.__APP__,b=[{key:"like",emoji:"👍",label:"Like",color:"#2563EB"},{key:"love",emoji:"❤️",label:"Love",color:"#F43F5E"},{key:"haha",emoji:"😆",label:"Haha",color:"#F59E0B"},{key:"wow",emoji:"😮",label:"Wow",color:"#F59E0B"},{key:"sad",emoji:"😢",label:"Sad",color:"#F59E0B"},{key:"angry",emoji:"😡",label:"Angry",color:"#EF4444"}],g=Object.fromEntries(b.map(e=>[e.key,e])),i=e=>String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;");function k(e){return e?`${_.urls.profileBase}/${encodeURIComponent(e)}`:"#"}function x(e){if(!e)return"";const t=Date.parse(e);if(Number.isNaN(t))return"";const s=Math.max(0,Date.now()-t),a=6e4,n=60*a,o=24*n;if(s>=5*o)return"";if(s<a)return"just now";if(s<n)return`${Math.floor(s/a)} min ago`;if(s<o)return`${Math.floor(s/n)} hr ago`;const r=Math.floor(s/o);return`${r} day${r===1?"":"s"} ago`}function L(e){if(!e||e[0]!=="#")return!0;const t=e.length===4?e.slice(1).split("").map(o=>o+o).join(""):e.slice(1,7),s=parseInt(t.slice(0,2),16),a=parseInt(t.slice(2,4),16),n=parseInt(t.slice(4,6),16);return(s*299+a*587+n*114)/1e3>=160}function C(e){const t=k(e==null?void 0:e.username),s=e!=null&&e.id?`data-user-id="${e.id}"`:"";if(e!=null&&e.profile_picture)return`<a href="${t}" ${s} data-user-link><img class="avatar" src="${i(e.profile_picture)}" alt=""></a>`;const a=i(((e==null?void 0:e.username)||(e==null?void 0:e.name)||"U").charAt(0).toUpperCase());return`<a href="${t}" ${s} data-user-link><span class="avatar">${a}</span></a>`}function u(e,t,s){const a=e.user||{},n=i(a.name||a.username||"User"),o=i(x(e.created_at_iso)),r=Number(e.views_count||0).toLocaleString(),l=k(a.username),d=a.id?`data-user-id="${a.id}"`:"";return`
    <div class="post-head">
      ${C(a)}
      <div class="who">
  ...
  `),t&&(a.push('<div class="post-menu-sep"></div>'),a.push(m("edit","Edit post","Update your post content",B())),a.push(m("delete","Delete post","Permanently remove this post",T(),"is-danger"))),`
  ...
  `}function Z(e){var a;if(!e)return;const t=(a=e.querySelectorAll)==null?void 0:a.call(e,"[data-adbot-slot]");if(!t||!t.length||document.getElementById("adbot-invoke"))return;const s=document.createElement("script");s.id="adbot-invoke",s.async=!0,s.dataset.cfasync="false",s.src=z,document.body.appendChild(s)}function J(){return`
    <article class="post-card ad-feed-card" data-ad-feed>
      <div class="post-head">
        ...
        <a class="ad-feed-out" href="${f}" target="_blank" rel="sponsored noopener" title="Learn more" aria-label="Learn more">
      ...
      ${y("feed-top")}
      <a class="ad-feed-cta" href="${f}" target="_blank" rel="sponsored noopener">
  ...
  `:"",d=o?`
    <div class="gallery-dots" data-gallery-dots>
      ${t.map((c,w)=>`<span class="dot${w===0?" active":""}"></span>`).join("")}
    </div>`:"";return`
    <article class="post-card image-card" data-post-id="${e.id}">
      ${u(e,"image","Photo")}
      ${n}
  ...
  `}function D(e){const t=e.book||{},s=e.view_url||"#",a=t.cover_url?`<img src="${i(t.cover_url)}" alt="" referrerpolicy="no-referrer"
       ...
       </button>`:"",r=t.description?`<p class="book-desc">${i(String(t.description).slice(0,220))}${String(t.description).length>220?"…":""}</p>`:"";return`
    <article class="post-card book-card" data-post-id="${e.id}" data-book-slug="${i(t.slug||"")}">
      ${u(e,"book","Book")}
      <div class="book-body">
        <a class="book-cover" href="${f}" target="_blank" rel="sponsored noopener" data-ad-cover>${a}</a>
        <div class="book-info">
  ...
  `}function Q(e){return e.type==="status"?q(e):e.type==="image"?I(e):e.type==="video"?O(e):e.type==="article"?U(e):e.type==="book"?D(e):""}function X(e){var o,r;const t=e.querySelector("[data-gallery]"),s=(r=(o=e.parentElement)==null?void 0:o.parentElement)==null?void 0:r.querySelector("[data-gallery-dots]"),a=e.querySelector("[data-gallery-counter]");if(!t)return;const n=()=>{if(!t.clientWidth)return;const l=Math.round(t.scrollLeft/t.clientWidth);s&&s.querySelectorAll(".dot").forEach((v,c)=>v.classList.toggle("active",c===l));const d=t.children.length;a&&(a.textContent=`${l+1} / ${d}`)};t.addEventListener("scroll",n,{passive:!0})}function ee(e,t){var o,r;if(!e||!t)return null;const s=t.embed_provider&&t.embed_id,a=(r=(o=t.media)==null?void 0:o[0])==null?void 0:r.url;if(s)e.innerHTML=`<div class="plyr__video-embed plyr-stage"
      ...
      </video>`;else return e.innerHTML="",null;const n=e.querySelector(".plyr-stage");return!n||typeof window.Plyr>"u"?null:new window.Plyr(n,{autoplay:!0,youtube:{noCookie:!0,rel:0,modestbranding:1}})}export{f as A,g as R,V as a,Y as b,X as c,K as d,Q as e,N as f,i as g,J as h,x as i,Z as j,L as k,ee as m,R as n,G as r,W as s};
